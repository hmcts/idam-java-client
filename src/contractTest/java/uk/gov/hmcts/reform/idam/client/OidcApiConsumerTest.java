...
public class OidcApiConsumerTest {
    ...
    // allows pooled connections to the previous mock server instance to expire before the next test
    @BeforeEach
    ...
    public V4Pact executeGetUserInfo(PactDslWithProvider builder) {
        ...
        return builder.given("I have obtained an access_token as a user", params)
                .uponReceiving("OIDC API returns user info to the client")
    ...
    @Pact(consumer = "idamClient")
    public V4Pact executeGetAccessTokenAndGet200(PactDslWithProvider builder) {
    ...
    @Test
    @PactTestFor(pactMethod = "executeGetAccessTokenAndGet200")
    ...
    @Test
    @PactTestFor(pactMethod = "executeGetAccessTokenResponseAndGet200")
    void verifyGetAccessTokenResponsePact() {

        TokenResponse tokenResponse = idamClient.getAccessTokenResponse("emCaseOfficer@email.net", "Password123");

        assertThat(tokenResponse).isNotNull();
        assertThat(tokenResponse.accessToken).isNotBlank();
        assertThat(tokenResponse.tokenType).isEqualTo("Bearer");
        assertThat(tokenResponse.scope).isEqualTo("openid roles profile search-user");
    }
